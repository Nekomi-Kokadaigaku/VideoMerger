...
class VideoMergeModel: ObservableObject {
  ...
  var mergeCommand: String {
    let inputs = videoFiles.map { "-i \"\($0.fileURL.path)\"" }.joined(
      separator: " ")
    ...
    process.terminationHandler = { [weak self] p in
      DispatchQueue.main.async {
        ...
        if p.terminationStatus == 0 {
          ...
          self.notifyUser(
            title: "合并完成", body: "已成功合并视频到: \(self.outputURL.path)")
          if let attributes = try? FileManager.default.attributesOfItem(
            atPath: self.outputURL.path),
  ...
  private func moveSourceFilesToTrash() {
    ...
    if !FileManager.default.fileExists(atPath: trashFolderURL.path) {
      do {
        try FileManager.default.createDirectory(
          at: trashFolderURL, withIntermediateDirectories: true, attributes: nil
        )
      } catch {
    ...
    for file in videoFiles {
      ...
      var destinationURL = trashFolderURL.appendingPathComponent(
  ...
  private func processDroppedFile(url: URL) {
    ...
    DispatchQueue.main.async {
      if !self.videoFiles.contains(where: { $0.fileURL == url }) {
        ...
        self.predictedMergedSize = self.videoFiles.compactMap(\.fileSize)
          .reduce(0, +)
      }
    }
  }
