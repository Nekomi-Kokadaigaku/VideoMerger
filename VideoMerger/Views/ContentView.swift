...
extension ContentView {
  ...
  private var videoFilesGroup: some View {
    GroupBox(label: videoFilesGroupLabel()) {
      ...
      .onDrop(
        of: [UTType.fileURL.identifier], isTargeted: nil,
        perform: model.handleFileDrop)
    }
  }
    ...
    HStack {
      ...
      if let predictedSize = model.predictedMergedSize, predictedSize > 0 {
        let sizeString = ByteCountFormatter.string(
          fromByteCount: Int64(predictedSize), countStyle: .file)
      ...
      if let mergedSize = model.mergedFileSize, model.mergeStatus == .success {
        let sizeString = ByteCountFormatter.string(
          fromByteCount: Int64(mergedSize), countStyle: .file)
        Text("（实际：\(sizeString)）")
          .foregroundColor(.secondary)
          .font(.caption)
      }
...
extension ContentView {
  ...
  private func selectSourceFolder() {
    FileSelector.selectFolder(
      allowedContentTypes: [UTType(filenameExtension: "flv")!],
      initialPathKey: "LastSelectedFolderPath"
    ) { url in
      if url.hasDirectoryPath {
        ...
        if !model.videoFiles.contains(where: { $0.fileURL == url }) {
          ...
          model.predictedMergedSize = model.videoFiles.compactMap(\.fileSize)
            .reduce(0, +)
        }
      }
    }
  ...
  private func selectOutputFolder() {
    FileSelector.selectFolder(
      allowedContentTypes: nil, initialPathKey: "LastOutputFolderPath"
    ) { url in
  ...
  private func videoItemView(_ video: VideoFile) -> some View {
    HStack(alignment: .top) {
      ...
      .contextMenu {
        Button("删除") {
          if let index = model.videoFiles.firstIndex(where: {
            $0.id == video.id
          }) {
            model.videoFiles.remove(at: index)
          }
